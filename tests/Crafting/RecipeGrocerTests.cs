...
public class RecipeGrocerTests
{
    ...
    public async Task GivenARecipeGrocer_WhenBreakingDownAnItem_WhenItHas2Levels_ThenReturnAllBrokenDown()
    {
        ...
        var expectedIngredients =
            recipe.AmountIngredient0 + (recipe.AmountIngredient0 * subRecipe.AmountIngredient0);
        Setup2LevelItemTest(itemID, recipe, subRecipe);
    ...
    public async Task GivenARecipeGrocer_WhenBreakingDownARecipe_WhenItHas2Levels_ThenReturnAllBrokenDown()
    {
        ...
        Assert.That(
            result.GetIngredientsSum(),
            Is.EqualTo(expectedIngredients.GetIngredientsSum())
        );
        Assert.That(
            result.GetIngredientsCount(),
            Is.EqualTo(expectedIngredients.GetIngredientsCount())
        );
    }
    ...
    private void Setup2LevelItemTest(int itemID, RecipePoco recipe, RecipePoco subRecipe)
    {
        _recipes.Get(recipe.ID).Returns(recipe);
        _recipes.Get(subRecipe.ID).Returns(subRecipe);
        _recipes.GetRecipesForItem(itemID).Returns(new List<RecipePoco>() { recipe });
        _recipes
        ...
        _recipes
            ...
            .Returns(new List<RecipePoco>() { subRecipe });
        _recipes.GetRecipesForItem(subRecipe.TargetItemID).Returns(Array.Empty<RecipePoco>());
    }
