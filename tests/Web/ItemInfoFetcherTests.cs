...
{
    private ItemInfoFetcher _fetcher;
    private IItemRepository _repo;
    private IMarketableItemIdsFetcher _marketableIdsFetcher;
    private ILogger<ItemInfoFetcher> _logger;

    [SetUp]
    public override void SetUp()
    {
        base.SetUp();
        _logger = Substitute.For<ILogger<ItemInfoFetcher>>();
        _repo = Substitute.For<IItemRepository>();
        _repo.GetAll().Returns(GetMultipleDbPocos());
        _marketableIdsFetcher = Substitute.For<IMarketableItemIdsFetcher>();
        _marketableIdsFetcher.GetMarketableItemIdsAsync().Returns(GetMultipleDbPocos().Select(i => i.Id).ToList());
        _fetcher = new ItemInfoFetcher(_repo, _marketableIdsFetcher, _logger, _client);
    }
    ...
    public void GivenWeCreateAItemInfoFetcher_WhenTheClientIsNotProvided_ThenWeCanUseADefaultClientInstead()
    {
        ...
        _fetcher = new ItemInfoFetcher(_repo, _marketableIdsFetcher, _logger);
