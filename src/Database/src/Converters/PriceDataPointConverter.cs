...
{
    Task<List<PriceDataPointPoco?>> ConvertAsync(PriceDataPointWebPoco dataPoint, int itemId, bool isHq);
}
...
{
    public async Task<List<PriceDataPointPoco?>> ConvertAsync(PriceDataPointWebPoco dataPoint, int itemId, bool isHq)
    {
        

        try
        {
            if (itemId < 1 || !dataPoint.HasValidPrice())
